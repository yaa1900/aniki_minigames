define([
    ...
    Application.prototype = Object.create( {} ,{
        ...
        /**
         * sonicmoovのicon自社広告webページのURL
         */
        createSMVIconAdURLWithSceneName: { value: function( sceneName ){
            var base = "http://s3-ap-northeast-1.amazonaws.com/bobby-sa-applink/ios/index.html";
            if( this.config.debug )
                base = "http://s3-ap-northeast-1.amazonaws.com/bobby-sa-applink/ios_stg/index.html";

            return base + "?from=" + this.config.urlscheme + "&scene=" + sceneName;
        }, writable:true  },
    ...
    TitleScene.prototype = Object.create( MiniScene.prototype ,{
        ...
        onPrepare: { value: function(){
            ...
            this._btnSonicmoov = new Button( this.config.title_btn_smv, "タイトル画面:sonicmoovボタン" );
            ...
            this._foreground.addChild( this._btnSonicmoov.display );
            this._btnSonicmoov.display.x = this.stage.stageWidth/2;
            this._btnSonicmoov.display.y = this.stage.stageHeight/2 + offset + span*2;
            this._btnSonicmoov.onTap = function(){ SonicmoovAd.showAppList("title"); };
        ...
        onShow: { value: function(){
            ...
            // show smv link
            if( !this.config.screenshotMode ) SonicmoovAd.showIconsIn( "title" );
        ...
        onHide: { value: function(){
            ...
            // hide smv link
            if( !this.config.screenshotMode ) SonicmoovAd.hideIcons();
        } , writable:true },
         ...
         * @type mini.Button
         */
        buttonSonicmoov: { get: function(){ return this._btnSonicmoov; } }
...
//            this._popupBtnRetry = new mini.Button( this.config.game_popup_btn_retry, "ゲーム画面:リトライボタン" );
    ...
    ResultScene.prototype = Object.create( MiniScene.prototype ,{
        ...
        onShow: { value:function(){
            ...
            // show smv link
            if( !this.config.screenshotMode ) SonicmoovAd.showIconsIn("result");
...
//            if( !groupName ) return tween;
     ...
     * @constructor
     */
    var SonicmoovAd = Object.create( {}, {

        /**
         * アイコン広告の表示中フラグ
         */
        _visibleIcon: {
            get: function(){ return this.hasOwnProperty("_visibleIcon")? this.__visibleIcon: false; },
            set: function( value ){ this.__visibleIcon = value; }
        },
        ...
        _cover:{
            get: function(){
                if( !this.__cover ) {
                    this.__cover = new Bitmap( new BitmapData( 1,1,true,0xff000000 ) );
                    this.__cover.width = application.stage.stageWidth;
                    this.__cover.height = application.stage.stageHeight;
                }
                return this.__cover;
            }
        },
         ...
         * webview使用
         */
        showIconsIn: { value: function( sceneName ){
            console.log("showIconsIn");

            this._visibleIcon = true;

            var suffix = "";
            if( application.config.debug ) suffix = "_stg";
            var os = "ios";
            if( app.isANDROID ) os = "android";

            var base = "http://s3-ap-northeast-1.amazonaws.com/bobby-sa-applink/"+os+suffix+"/index.html";

            var url = base + "?from=" + application.config.urlscheme + "&scene=" + sceneName;

            var prevWebviewArea = this._webviewArea || {};
            var sizeRatio = innerWidth < innerHeight? innerWidth/640: innerHeight/640;
            this._webviewArea = Object.create( {}, {
                rect: { value: new Rectangle( 0, -100*sizeRatio, 640*sizeRatio, 100*sizeRatio ) },
                x: { get: function(){ return this.rect.x; }, set: function( value ){ this.rect.x = value; this.layout(); } },
                y: { get: function(){ return this.rect.y; }, set: function( value ){ this.rect.y = value; this.layout(); } },
                layout: { value: function(){ webview.setLayout( this.rect ); } }
            });
            ...
            webview.onload = function(){
                ...
                // android
                if( app.isANDROID ) {
                    setTimeout( function(){ self._webviewArea.y = 0; },100 );
                    return;
                }

                mini.Tween.removeTweens( prevWebviewArea );
                mini.Tween
                    .get( self._webviewArea, { useTicks: true })
                    .to( { y:0 }, 12, mini.Ease.sineOut );
            };
            this._webviewArea.layout();
        } },
        /**
         * 自社アイコン広告を非表示
         * webview使用
         */
        hideIcons: { value: function(){

            this._visibleIcon = false;

            webview.onload = null;

            // android
            if( app.isANDROID ) {
                setTimeout( function(){ webview.hide(); },100 );
                return;
            }

            mini.Tween.removeTweens( this._webviewArea );
            var sizeRatio = innerWidth < innerHeight? innerWidth/640: innerHeight/640;
            mini.Tween
                .get( this._webviewArea, { useTicks: true })
                .to( { y:-100*sizeRatio }, 9, mini.Ease.sineOut)
                .call( function(){ webview.hide(); } );
        } },
         ...
         * webveiw使用
         */
        showAppList: { value: function( sceneName ){

            // TODO やっつけすぎ 処理負荷によってはバグる
            if( app.isIOS && this._visibleIcon ) {
                this.hideIcons();
                var self = this;
                setTimeout( function(){ self.showAppList( sceneName ); self._visibleIcon = true }, 200 );
                return;
            }
            ...
            var base = "http://s3-ap-northeast-1.amazonaws.com/bobby-sa-applink/list_page"+suffix+"/index.html";
            var url = base + "?from=" + application.config.urlscheme + "&scene=" + sceneName;

            var prevWebviewArea = this._webviewArea || {};
            this._webviewArea = Object.create( {}, {
                rect: { value: new Rectangle( 0, innerHeight, innerWidth, innerHeight ) },
                x: { get: function(){ return this.rect.x; }, set: function( value ){ this.rect.x = value; this.layout(); } },
                y: { get: function(){ return this.rect.y; }, set: function( value ){ this.rect.y = value; this.layout(); } },
                layout: { value: function(){ webview.setLayout( this.rect ); } }
            });
            ...
            // cover
            application.stage.addChild( this._cover );
            this._cover.alpha = 0;
            mini.Tween.get( this._cover, {useTicks:true}).to({ alpha:0.7 },30);

            // webview
            var self = this;
            webview.load( url );
            webview.onload = function(){ webview.show(); };
            webview.onmessage = function( e ){

                if( e.message === "close" ) {

                    application.stage.removeChild( self._cover );
                    // hide
                    webview.onmessage = null;
                    mini.Tween.removeTweens( self._webviewArea );
                    mini.Tween
                        .get( self._webviewArea, { useTicks: true })
                        .to( { y:innerHeight }, 12, mini.Ease.sineInOut)
                        .call( function(){
                            if( self._visibleIcon )
                                self.showIconsIn( sceneName );
                            else
                                webview.hide();
                        } );
                    return;
                }
                if( e.message !== "ready" ) return;// TEST
                mini.Tween.removeTweens( prevWebviewArea );
                mini.Tween
                    .get( self._webviewArea, { useTicks: true })
                    .to( { y:0 }, 18, mini.Ease.sineInOut )
                    .wait(0);
            };
            this._webviewArea.layout();
